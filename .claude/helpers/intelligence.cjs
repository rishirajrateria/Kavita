...
function resolveProjectRoot(startDir) {
  ...
  while (true) {
    if (fs.existsSync(path.join(dir, ".git")) || fs.existsSync(path.join(dir, ".claude-flow"))) {
      return dir;
    }
...
const LEGACY_PENDING_PATH = path.join(
  process.cwd(),
  ".claude-flow",
  "data",
...
function ensureDataDir() {
  ...
  if (
    path.resolve(LEGACY_PENDING_PATH) !== path.resolve(PENDING_PATH) &&
    ...
    try {
      const existing = fs.existsSync(PENDING_PATH)
        ...
        : new Set();
      const recovered = fs
        .readFileSync(LEGACY_PENDING_PATH, "utf-8")
        .split("\n")
      ...
      /* migration is best-effort; never block hook execution */
    }
  }
}
...
function storeFingerprint(entries) {
  ...
  const parts = entries
    .map(
      (e) =>
        `${e.id || e.key || ""}:${fingerprintContent(e.content || e.summary || e.value || "")}`,
    )
    .sort();
...
function clip(text, max, label) {
  ...
  if (CLIP_DEBUG)
    process.stderr.write(
      `[INTELLIGENCE] WARN: truncated ${label || "value"} from ${text.length} to ${max} chars\n`,
    );
...
function init() {
  ...
  if (
    graphState &&
    graphState.nodeCount === deduped.length &&
    graphState.contentFingerprint === currentFingerprint
  ) {
  ...
  try {
    if (fs.statSync(PENDING_PATH).size > 512 * 1024) {
      ...
      if (lines.length > 2000)
        fs.writeFileSync(PENDING_PATH, lines.slice(-2000).join("\n") + "\n", "utf-8");
    }
  } catch (e) {
    /* non-fatal */
  }
