...
async function gracefulExit(signal) {
  ...
  shuttingDown = true;
  if (DEBUG)
    process.stderr.write(`[AutoMemory] received ${signal}, flushing backend before exit\n`);
  try {
    if (activeBackend && typeof activeBackend.shutdown === "function")
...
async function doStatus() {
  ...
  console.log(
    `  Package:        ${memPkg ? "✅ Available" : "❌ Not found — self-learning DISABLED (fix: npm i -D @claude-flow/memory)"}`,
  );
  console.log(
    `  Resolver:       ${hasSidecar ? "✅ .claude-flow/memory-package.json" : "⏸ no sidecar (run: npx ruflo@latest doctor --fix)"}`,
  );
  console.log(
